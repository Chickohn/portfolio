...
interface TotalsPanelProps {
  ...
  onToggleVatNotRegistered: (value: boolean) => void;
    ...
    <aside className="rounded-xl border border-slate-200 bg-white p-5 text-slate-900 shadow-sm xl:sticky xl:top-24">
      ...
      <p className="mt-1 text-sm text-slate-500">
        {showShipping
          ...
          : "Adjust sections and VAT settings in the layout panel."}
      </p>
      ...
      <div className="mt-5 space-y-3">
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          <span className="font-medium">
            {vatNotRegistered ? "Not VAT registered" : formatCurrency(totals.vatTotal)}
          </span>
        </div>
        ...
        <label className="flex cursor-pointer items-center gap-2 text-sm text-slate-700">
          <input
            ...
            onChange={(event) => onToggleVatNotRegistered(event.target.checked)}
        ...
        <div className="border-t border-slate-200 pt-3">
          <div className="flex items-center justify-between">
            <span className="font-semibold">
              {docType === "Invoice" ? "Total due" : "Grand total"}
            </span>
